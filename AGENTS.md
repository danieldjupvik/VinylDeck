...
3. Open PR targeting `main` and let CI run
4. **Squash merge** the PR
5. release-please opens the release PR; squash merge it to deploy production
...
- **TanStack Query** for server state with IndexedDB query-cache persistence
...
TanStack Router uses file-based routes in `src/routes/`.
...
- **Zustand store** (`profile-cache-store.ts`) - User profile cache (localStorage, instant on load)
...
`HydrationProvider` tracks IndexedDB restoration. Use `useHydrationGuard` for expensive persisted queries (for example collection fetches) to avoid duplicate requests during hard reload hydration. Skip it for lightweight metadata polls.
...
- **preferences-store.ts** - View mode, avatar source, Gravatar email/url, lastSeenVersion
- **profile-cache-store.ts** - User profile cache (id, username, avatar_url, email)
...
- **Other server query data** - Persisted in IndexedDB as needed
- **Note** - User profile cache is persisted in localStorage via `useProfileCacheStore` (not TanStack Query)
...
| `vinyldeck-oauth-request-secret` | Temporary OAuth request token secret |
| `vinyldeck-redirect`             | Post-login redirect URL              |

**IndexedDB:**

- TanStack Query cache (collection and other query data)
...
`@tanstack/router-generator` still resolves a Zod v3/v4-incompatible path in some Bun installs. Keep `scripts/patch-tanstack-zod-imports.mjs` wired in `postinstall` and `build` until the upstream fix ships.

**When to remove:** Once TanStack Router ships the fix, remove `scripts/patch-tanstack-zod-imports.mjs` and the `patch:tanstack-zod` script references from `package.json`.
