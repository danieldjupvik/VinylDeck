...
must_haves:
  truths:
    ...
    - 'Animations use shadcn built-in transitions (no custom staggered reveals)'
  artifacts:
  ...
  key_links:
    ...
    - from: 'src/components/changelog/changelog-content.tsx'
      to: 'ChangelogEntry'
      via: 'entry rendering in category lists'
  ...
  animation_note: >
    CONT-05 (smooth entry animations) is satisfied by shadcn's built-in Dialog/Drawer/Accordion
    animations. Per CONTEXT.md: "Content appears immediately with modal (no staggered reveal)"
    and "Trust shadcn components to handle prefers-reduced-motion". No custom animation work needed.
---
...
Output: Three changelog components - the responsive modal wrapper, the truncatable entry component, and the content layout.
</objective>
...
- Rely on shadcn's built-in open/close animations - no custom transitions needed
...
<task type="auto">
  <name>Task 2: Create changelog entry component with truncation</name>
...
This component will be used by ChangelogContent (Task 3) for each entry in a category.
</action>
<verify>
...
<task type="auto">
  <name>Task 3: Create changelog content component with header, categories, and footer</name>
...
6. **Render each entry using the ChangelogEntry component from Task 2**
7. Render "Got it" dismiss button in footer
...
**Entry rendering:** Import ChangelogEntry from `./changelog-entry` and use it to render each entry:

```typescript
import { ChangelogEntry } from './changelog-entry'

// Inside category rendering:
{entries.features?.map((entry, index) => (
...
# Verify ChangelogEntry import and usage
grep "ChangelogEntry" src/components/changelog/changelog-content.tsx
...
```
  ...
  <done>
ChangelogContent component renders header with version + date, category sections with Badge + icon for each non-empty category, entries via ChangelogEntry component, and "Got it" dismiss button. Uses formatChangelogDate, Badge variants, Lucide icons, and ChangelogEntry for truncatable entries.
  </done>
</task>
...
2. ChangelogEntry handles text truncation with "Show more" toggle
3. ChangelogContent displays version header, categories with badges, entries via ChangelogEntry, dismiss button
...
- Long entries show truncation with expand toggle (via ChangelogEntry)
