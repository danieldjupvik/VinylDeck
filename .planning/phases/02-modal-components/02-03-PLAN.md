...
must_haves:
  truths:
    ...
    - 'Accordion expand/collapse uses shadcn built-in animations'
  artifacts:
  ...
  key_links:
    ...
    - from: 'src/components/changelog/version-accordion.tsx'
      to: 'ChangelogContent'
      via: 'content rendering inside accordion items'
      pattern: 'from.*changelog-content'
...
5. **Import ChangelogContent from `@/components/changelog/changelog-content` and render it inside each AccordionContent**
...
**Content rendering:** Import and use ChangelogContent from Plan 02-02:
...
Rely on shadcn Accordion's built-in expand/collapse animations - no custom transitions needed.
...
# Verify ChangelogContent import
grep "from.*changelog-content" src/components/changelog/version-accordion.tsx
...
```
  ...
  <done>
VersionAccordion component displays multiple versions with shadcn Accordion, renders ChangelogContent inside each AccordionContent. Latest version expanded by default (defaultValue="item-0"), older versions collapsed. 5+ versions shows first 3 with "+ N older versions" expand button.
  </done>
</task>
