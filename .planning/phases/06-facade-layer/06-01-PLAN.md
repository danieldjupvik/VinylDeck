...
must_haves:
  truths:
    ...
    - 'Data operations (getIdentity, getCollectionReleases, getUserProfile) route through discojs wrapper with retry'
...
- Uses `Discojs` from discojs with optional auth token
- Wrap all calls with `withRateLimitRetry` from retry.ts
- Methods:
  - `getIdentity()` - requires auth, throws DiscogsAuthError if no tokens
  - `getCollectionReleases(username, folderId?, options?)` - requires auth
  - `getUserProfile(username)` - requires auth
...
- User agent: `VinylDeck/${process.env.npm_package_version ?? '1.0.0'}`
...
<done>
client.ts exports createDataClient with getIdentity, getCollectionReleases, getUserProfile
