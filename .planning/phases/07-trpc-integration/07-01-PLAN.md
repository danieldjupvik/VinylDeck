...
Two changes required — interface AND implementation:

1. **Update the `DataClient` interface** (lines 59-72 in client.ts): Add `getCollectionMetadata(username: string): Promise<{ totalCount: number }>` as a new method signature alongside the existing `getIdentity`, `getCollectionReleases`, and `getUserProfile` signatures. The interface signature must match the implementation signature exactly.

2. **Add the implementation** in the `createDataClient` return object: Add a `getCollectionMetadata` method that:
   - Requires tokens (throw `DiscogsAuthError` if not present, same pattern as other methods)
   - Calls `wrapCall('getCollectionMetadata', ...)` wrapping `client.listItemsInFolderForUser(username, 0, undefined, { page: 1, perPage: 1 })`
   - Extracts `pagination.items` from the response and returns `{ totalCount: pagination.items }`
...
<done>
Error mapper function exists and handles all facade error types. DataClient interface and implementation both include getCollectionMetadata that returns `{ totalCount: number }`.
</done>
</task>
