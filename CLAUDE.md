...
### ⚠️ CRITICAL: User-Facing Changelog

**The app has two separate changelogs — do not confuse them:**

- **`CHANGELOG.md`** — auto-generated by release-please from commit messages (never edit manually)
- **`src/data/changelog.ts`** + translation files — curated user-facing changelog shown in the "What's New" modal

**User-facing changelog entries are added on the release-please branch before merging a release.** This is where the version number and date are already known.

**On the release-please branch, before merging:**

1. Review the auto-generated `CHANGELOG.md` for user-facing changes (features, fixes)
2. Add a new entry at the top of `src/data/changelog.ts` with:
   - `version` from `package.json` (e.g., `'0.5.0-beta.1'`)
   - `date` from `CHANGELOG.md` (e.g., `'2026-02-18'`)
   - Entry keys for each user-facing change
3. Add matching translation keys in both:
   - `src/locales/en/translation.json` under `changelog.{version_underscored}.*`
   - `src/locales/no/translation.json` with Norwegian translations
4. Write translations as user-friendly descriptions, not commit messages

**Translation key pattern:** `changelog.{version_underscored}.{category}_{index}`
Example: `changelog.0_4_0_beta.feature_1`

**Skip changelog entries for:** dependency bumps, refactors, CI changes, internal tooling — anything invisible to users.
